...
class ChannelAttention(nn.Module):
    def __init__(self, input_dim, reduction_ratio=8):
        ...
        reduced_dim = input_dim // reduction_ratio
...
class ColorAdjustment(nn.Module):
    ...
    def forward(self, x):
        color_adjustment = self.fc(x.mean(dim=1, keepdim=True))
        color_adjustment = self.sigmoid(color_adjustment)
        
        return x * color_adjustment

    
class ChannelResampler(nn.Module):
    def __init__(self, query_dim=1024, reduction_ratio=8, use_color_adjust=True):
        super().__init__()

        self.channel_attention = ChannelAttention(query_dim, reduction_ratio=reduction_ratio)
        self.ff = FeedForward(query_dim, glu=True)

        self.norm1 = nn.LayerNorm(query_dim)
        self.norm2 = nn.LayerNorm(query_dim)

        self.color_adjustment = ColorAdjustment(query_dim)
        self.use_color_adjust = use_color_adjust

    def forward(self, x):
        x = x + self.channel_attention(self.norm1(x))
        x = x + self.ff(self.norm2(x))

        if self.use_color_adjust:
            x = self.color_adjustment(x)
